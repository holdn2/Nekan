...
- **Apple로 로그인해서 동기화** — iPhone에서 Apple로 로그인하셨다면 이쪽입니다
...
macOS에서는 이 문서의 `Ctrl`을 전부 `Cmd`로 읽으면 됩니다 — 앱은 그 OS의 조합키를 씁니다.
가이드 탭은 다릅니다 — **단축키가 나오는 자리마다 `Ctrl / Cmd`처럼 둘을 나란히** 적습니다.
가이드는 지금 쓰는 기계가 아니라 다른 기계 것을 확인하려고 읽는 일이 많아서입니다. 앱 안의
다른 안내(설정 패널의 내보내기 힌트)는 지금 쓰는 조합키만 적습니다.
...
**로그인하면 여러 기기에서 같은 목록을 씁니다.** Google이나 Apple 계정 하나면 되고, 로그인 창은
앱 안이 아니라 **평소 쓰는 브라우저**에서 열립니다 (어디를 눌러 로그인하는지는
[가이드 탭](#가이드-탭)에 있습니다). **Google과 Apple은 이메일이 같을 때만 같은 목록을 씁니다** —
다른 기기와 같은 쪽으로 로그인하세요.
...
- **한 페이지에 20개씩 그립니다** (행 하나가 약 180µs라, 검색어를 칠 때마다 끊기지 않게 하려는 상한). 나머지는 목록 아래의 페이지 번호로 이어지고 **항목이 지워지는 것은 아닙니다.**
- 다 꺼내기에서 바로 지운 항목은 소속이 없어서 **양쪽 휴지통에 모두** 나타납니다. `휴지통 비우기`는 **지금 탭이 들고 있는 전체**를 지웁니다 — 보이는 한 페이지가 아니라, 선택한 매트릭스의 기록 전부입니다.
...
**앱을 켠 사람이 읽는 사용 설명서입니다.** 아이젠하워 매트릭스가 무엇인지와 **이 앱을 쓰는 법 전부**가 여기 있습니다 — 단축키까지 포함해서요. **그 탭만 보고도 앱을 다 쓸 수 있어야 한다**는 것이 이 문서와의 경계입니다.

**두 층입니다.** 열면 보이는 것은 네 칸을 줄여 그린 그림과 쓰는 순서 셋, 기본 조작 넷뿐이고, 나머지는 그 아래 다섯 개의 접힌 절에 들어 있습니다(아이젠하워 매트릭스란? · 할 일과 다 꺼내기 · 마감일과 메모 · 기록과 내보내기 · 창 · 단축키 · 동기화). **접었을 뿐 줄인 것은 아닙니다** — 조작 설명은 한 문단도 빠지지 않았습니다.

- **맨 아래에 업데이트 상태와 실행 중인 버전**, 릴리스 노트 링크가 있습니다 (링크는 기본 브라우저로 열립니다).
