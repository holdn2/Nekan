...
src/               쓰는 곳. TypeScript다 — 도는 것은 out/이다 (아래 "빌드")
  ...
  renderer/        React. Vite가 묶는다 — import 경로에는 여전히 `.js`를 쓰고
    ...
    selection.ts   어느 task의 메모를 보고 있나. 매트릭스·타이틀바가 뷰를 import하지
                   않고 이걸 본다
    drafts.ts      쓰는 중인 메모를 패널 밖에서 저장하게 하는 등록소. 패널을 치우는
                   코드(선택·바 모드·창 닫기)가 `flushDrafts()`를 먼저 부른다
...
- **sync는 로컬 task를 절대 지우지 않는다.** 로그아웃해도 `data.json`은 그대로다. 커서와
  ...
  **이 규칙은 폰에도 해당된다** (2026-09-12, #122): `apps/mobile`의 `stopSync()`가 타이머와
  화면만 끄고 `syncState`를 안 건드려서, 커서가 로그아웃을 살아남았다. 같은 계정으로 다시
  로그인하면 `useAccount()`가 **계정이 같다고 그냥 빠져나가므로** 아무도 안 잡아 주고,
  pull이 "N 이후"를 물어 **로그아웃 이후 변경분만** 받아 왔다.
  **그리고 커서를 버려야 하는 자리가 하나 더 있다: 보드를 통째로 비울 때.**
  첫 로그인의 "합치기"를 끄면(`replace`) 보드가 비는데, 커서가 남아 있으면 다시 채울 것을
  아무것도 못 받아 **빈 화면이 그대로 굳는다.** 지금은 폰의 `adoptLocalTasks`와 데스크톱의
  `syncAccount(userId, fresh)`가 각각 버린다. **합치기(기본값)로는 안 보인다** — 로컬 보드가
  남아 있어서 안 내려와도 티가 안 난다. 스위치를 꺼야 드러난다.
  **세션이 사라지는 길은 로그아웃만이 아니다.** 갱신이 4xx로 실패하면 `api/`의 `forget()`이
  세션을 그 자리에서 버리는데 **api는 sync를 모른다.** 그래서 `syncAccount(null)`은 평범한
  로그아웃만 막고, 토큰이 죽은 뒤에는 **`merge` 로그인도 뚫린다.** 지금은 **루프가 "세션이
  없다"를 발견한 자리에서** 잊는다(양쪽 앱) — 발견하는 것이 루프뿐이라 거기가 유일한 자리다.
  **그리고 커서를 실행 중인 pull 밑에서 갈아끼우면 경주가 된다**: 그 pull이 돌아와
  `Math.max()`로 옛 커서를 도로 올린다. 폰은 세션 epoch로 막고 있었고 데스크톱은 `generation`
  으로 막는다 — await 뒤마다 자기 세대인지 묻고, 아니면 쓰지 않고 `schedule(0)`으로 다시 돈다.
- **동기화를 깨우는 것은 일곱이고, 손으로 누르는 것은 그중 하나일 뿐이다** (2026-09-07):
  편집 후 3초 · **창 포커스 복귀** · **절전 복귀** · 폰의 앱 복귀 · **`online` 이벤트**(PC) ·
  60초 하트비트 · 6시간 전체 재조회. 사용자가 못 박은 기준이 있다 — _"네트워크만 연결되어
  있다면 자동으로 되어야 해"_ — 그래서 **어떤 것도 "버튼을 눌러야만" 되게 만들지 않는다.**
  **`RETRY_MS`가 `IDLE_MS`를 넘어서면 안 된다.** 한때 300초까지 올랐는데, 긴 대기가 아끼는
  요청은 어차피 네트워크가 없어 나가지도 못하는 것이고 대신 잃는 것이 "터널을 빠져나왔는데
  5분을 기다린다"였다. 지금 사다리는 `[5s, 20s, IDLE_MS]`이고, **실패 중인 동기화가 정상
  동기화보다 느리게 회복하지 않는다**는 것이 그 배치의 뜻이다.
  `online`은 렌더러가 듣는다 — 메인에는 연결 상태를 알려주는 것이 없다(`powerMonitor`는
  절전이지 연결이 아니다). 그리고 그것은 **"이 기계에 인터페이스가 생겼다"이지 "서버가
  답한다"가 아니다**: 공용 와이파이 로그인 페이지에서도 뜨고, 그러면 한 번 실패하고 사다리가
  다시 시작한다.
  **손 버튼(`sync:now`)의 값은 즉시성이 아니라 백오프 리셋이다.** 실측으로 한 번 도는 데
  169ms라 "지금 눌렀다"는 사실이 화면에 남지 않는다 — 그래서 아이콘의 바쁜 모습을 600ms
  붙든다(**글자는 사실대로 두고 아이콘만**).
...
- **색 리터럴은 `src/shared/theme.ts` 밖에 있으면 안 된다** (2026-08-26, PR #92). `palette.css`도
  ...
  **주석에 이슈 번호를 `#136`처럼 적으면 걸린다** — 숫자 셋이 16진수 색으로 읽힌다(`#123`·`#1234`
  같은 3·4·6·8자리 전부). 2026-09-15에 코드 네 파일이 그걸로 실패했다. 코드 주석에서는 `issue 136`
  으로 적는다(`.md`는 이 검사가 읽지 않는다).
...
- 인박스 행에는 마감일·완료·메모가 **의도적으로 없다**(`inboxItemEl`). 그래서 `selectedTask()`가
  ...
  패널이 열린 채 남는다.
- **메모는 쓰는 중에 저장된다 — 입력이 1초 멈추거나 패널이 치워지는 순간** (#136, 2026-09-15).
  패널을 치우는 코드(선택 변경·바 모드·창 닫기)는 뷰를 import할 수 없어서, 패널이 "쓰던 것을 써라"를
  `renderer/drafts.ts`에 등록하고 그쪽이 `flushDrafts()`를 **먼저** 부른다. **패널을 치우는 길을 새로
  만들면 여기서 먼저 불러야 한다** — 안 부르면 그 길로 떠날 때만 조용히 사라진다. 빈 칸은 저장하지 않고,
  취소·Esc는 편집을 연 순간의 메모로 되돌리되 **마지막 멈춤이 쓴 글이 그대로일 때만** 되돌린다 —
  그 사이 동기화가 바꾼 메모를 옛 글로 덮으면 다른 기기의 편집이 모든 기기에서 사라진다.
  **폰 상세 화면도 같은 1초 규칙이고, 입력이 있었던 칸만 쓴다**(`apps/mobile/app/task/[id].tsx`) —
  화면의 칸은 열 때 떠 온 사본이라 손대지 않은 칸을 쓰면 같은 방식으로 원격 편집을 지운다. 창을 닫는 두 길(타이틀바 버튼 · `beforeunload`)은
  2026-09-15에 앱을 띄워 1초 안에 닫고 `data.json`을 읽어 확인했다 — 변경 전 코드는 둘 다 `null`이었다.
...
apps/mobile/
  ...
  speech/         transcript(순수) · use-dictation(훅) + test/
                  **온디바이스 전용이다**: `requiresOnDeviceRecognition: true`이고
                  네트워크 인식으로 **폴백하지 않는다.** 할 일 목록은 아직 안 한 일의
                  목록이라, 오래된 기기에서 기능을 살리려고 그 음성을 남의 서버로
                  조용히 보내는 거래는 하지 않는다 — 못 하는 기기는 버튼이 꺼진다.
                  **받아쓰기는 제출하지 않는다.** 글자는 타이핑과 같은 자리에 들어가고
                  추가는 사람이 누른다 — 이름과 숫자를 자주 틀려서, 스스로 채우는 목록은
                  곧 고칠 것들의 목록이 된다.
                  **부분 결과는 덧붙이는 것이 아니라 같은 문장의 더 긴 판이다.** 그래서
                  누를 때의 글자를 `base`로 붙들고 매번 그것과 합성한다(`composeDictation`) —
                  이어붙이면 "우유 우유 사기"가 된다
...
**줄 수는 크기마다 정해 두지 않는다** (2026-09-26). 같은 "중간"이 폰마다 높이가 달라서(mini 155 · 6.1인치 158 ·
6.7인치 170pt), 고정 개수는 한 폰에서 아래를 비우고 다른 폰에서 헤더를 잘랐다 — 6.1인치 중간이 실제로 12pt 넘칠
참이었다. `RowFit`이 `GeometryReader`의 높이로 들어가는 줄 수를 세고 남는 몇 pt를 제목 아래와 줄 사이에 나눈다.
**간격은 크기마다 다르다**(`Rows.medium`·`Rows.large`): 중간은 어느 폰에서나 3줄을 지키려고 촘촘하고, 큰 크기는 오탭을
줄이려고 넓다(사용자 결정 2026-09-26). **줄 높이·간격을 바꾸면 `Scale`·`Rows`와 플레이그라운드(문서 072)의
`SCALE`·`ROWS`를 같이 고칠 것** — 이 계산은 Swift에만 있어 어떤 테스트도 안 본다. 체크 원은 줄의 **오른쪽 끝**이다(앱 목록도) — 오른손 엄지.
**위젯은 iOS 시스템 색이 아니라 앱의 팔레트 역할로 그린다**(`panel`·`panel-2`·`accent`… `WIDGET_ROLES`).
`Color(hex:)`가 `#rrggbb`만 읽어서 알파가 붙은 역할(`*-soft`·`ring`)을 넣으면 조용히 회색이 된다 — 피드 테스트가 막는다.
...
**앱을 여는 곳은 스위치 오른쪽의 `arrow.up.forward.app` 하나다**(`Link`, 2026-09-26). 그 밖은 누르면 **아무 일도
없다** — 위젯 전체 밑에 아무것도 안 하는 버튼(`NothingIntent`)을 깔았다. 위젯은 기본적으로 어디를 눌러도 앱을 열고 그걸
끄는 설정이 없는데, 버튼은 위젯 안에서 인텐트만 돌리고 앱을 열지 않기 때문이다. 이유는 오탭이다: 줄마다 체크 원이
생기자 살짝 빗나간 탭이 앱을 열었다. **할 일 글자를 눌러 상세를 여는 길도 일부러 없다**(사용자: _"누르면 이동하는 것은
잘못누르면 앱이 열리는 이슈를 방지한 방법이 효과가 떨어지게 될 것 같아"_). 첫 피드 전에는 이 밑판이 없어서 누르면 앱이
열린다 — 그때 할 수 있는 일이 그것뿐이다. **Nekan 로고는 장식이다**(한때 앱을 열었지만 로고는 누를 것처럼 안 보였다).
그림은 `expo-target.config.js`의 `images`가 prebuild 때 `targets/quick/Assets.xcassets`에 쓰고(git 무시),
...
**옮기기는 드래그만이 아니다. 상세 화면의 칩이 같은 일을 한다** (2026-09-21).
`app/task/[id].tsx`의 `MoveRow`가 네 분면과 **다 꺼내기**를 칩으로 내놓고, 다 꺼내기에 있는 할 일도
같은 줄로 분면에 보낸다 — 그 전까지 다 꺼내기로 되돌리는 길은 드래그뿐이었고, **끌지 못하는 사람은
한 번 넣으면 못 꺼냈다.** 칩은 `moveToTop`·`unfileTask`를 그대로 부르므로 드래그와 결과가 같아야 한다:
**마감일·메모는 건드리지 않고**(다 꺼내기는 숨길 뿐이다) `space`만 `spaceFor()`가 정한다.
다 꺼내기 칩에는 점이 없다 — 점은 "어느 분면"을 뜻하고 다 꺼내기는 분면이 아니다.
`apps/mobile/store/test/move.test.ts`가 지킨다(제스처는 어떤 테스트도 돌리지 않으므로 그 경로의
유일한 안전망이다).
...
어느 계산이 맞는지 가리지 못해서, **키보드가 올라올 때 입력칸 바닥(`measureInWindow`)과 키보드 윗변
(`endCoordinates.screenY`)을 둘 다 창 기준으로 재고** 그 차이 + `KEYBOARD_GAP`만큼 아래 패딩(`lift`)을
준다. **다시 KAV의 오프셋으로 돌리지 말 것.** 잴 때는 이미 준 `lift`를 더해 되돌린다 — 키보드가 떠 있는
동안 두 번째 이벤트(키보드 높이 변화)가 첫 번째 위에 쌓이면 안 된다.
**드롭 목표가 낡는 함정은 그대로다**: `onLayout`은 카드가 "부모 안에서" 움직일 때만 답하는데, 패딩이
패널을 줄이면 카드는 **창 기준으로만** 옮겨간다. 그대로 두면 놓은 행이 **한 칸 위 분면으로 들어간다.**
그래서 `lift`가 바뀔 때마다 카드를 다시 잰다. 드롭 목표를 쓰는 화면에 높이를 바꾸는 것을 새로 더할 때마다
같은 질문을 할 것.
**끌리는 사본도 같은 함정의 다른 얼굴이다** (2026-09-22). 손가락 좌표는 **창 기준**으로 오는데 사본은
매트릭스 화면 **안**에 그려지고, 그 화면은 창 맨 위가 아니라 **앱 헤더 아래**에서 시작한다. 그래서 사본이
정확히 헤더 높이만큼 손가락 아래로 처졌고, 놓일 자리 선은 창 기준끼리 비교해서 멀쩡했다 — 기기에서
"선은 따라오는데 항목은 한참 아래"로 신고됐다. 지금은 화면의 창 기준 시작점(`originX`·`originY`)을 재서
뺀다. **레이아웃 때와 끌기를 시작할 때 둘 다** 잰다 — 헤더가 자라면 화면 전체가 움직여도 `onLayout`은
답하지 않는다. **창 좌표와 화면 좌표를 섞는 코드를 새로 쓰면 이 질문부터 할 것.**
...
`migrateLegacyStore()`가 옛 데이터를 끌어온다).
**`settings.startupChoice`에 아무 값이나 넣으면 안 된다.** `needsStartupChoice()`가 인정하는
것은 `"sync"`와 `"local"` 둘뿐이라, 그 밖의 값은 **첫 실행 카드가 떠 있는 상태**로 판정된다 —
카드는 클릭을 삼키고 `collapse()`는 아예 거부한다(`main/window/fold.ts`). 2026-09-07에
`"fresh"`를 심어 놓고 "바 모드가 안 된다"를 앱 결함으로 읽을 뻔했다. **증상이 조용해서
하니스 문제로 보이지 않는다.** GUI 클릭은 좌표로 쏘지 말고 CDP로
...
두 계정으로 기기 두 대를 흉내 내 서른 몇 가지를 본다
(LWW·동점·두 갈래·묘비·삭제 차단·RLS 격리·커서·페이지 넘기기·RPC 권한).
**개수를 기억으로 적지 말 것 — 마지막 줄이 답이다.** 여기 적힌 숫자도 그날의 것이고,
한동안 낡은 채로 있었다: 2026-09-13에 이 문단이 23을 말하는 동안 실제로는 35였다.
오래 19였고, 2026-09-07에 넷(늦게 온 휴지통이 묘비를 못 지우는지 둘 · 자기 찌꺼기가
정말 묻혔는지 둘), 2026-09-13에 매장·묘비 규칙으로 다시 늘어 **35**가 됐다 —
그중 여섯은 찌꺼기 정리를 확인하는 루프 하나에서 나온다(`check(` 호출 자리를 세면
서른 몇이 아니라 서른쯤으로 보이는 이유다).
**마지막 것이 늘어난 이유가 중요하다**: 그 정리 push는 원래 아무도 안 보고 있었고,
`state_at`을 안 올려 조용히 기각되면서 **글자 없는 행 하나를 계정에 남겼다.**
검사 없는 정리는 정리가 아니다.
