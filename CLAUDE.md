...
**`shared/core.js`에 export를 더하면 `renderer/core-bridge.js`에도 적어야 한다.** 그쪽은 손으로
적은 재수출 목록이라 자동으로 따라오지 않는다. 빠뜨리면 import가 실패해 `app.js`의 `init()`이 죽고,
**증상은 오류 화면이 아니라 `<body class="booting">`에 멈춘 앱**이다 — 데이터가 안 보이는 것처럼
읽혀서 엉뚱한 곳을 뒤지게 된다(2026-08-19에 `MIN_INBOX_PX`로 실제로 겪었다).
