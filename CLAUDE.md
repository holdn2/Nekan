...
- **가이드 탭은 문단이 키 하나다** (`guide.*`, 56개). 목록 항목도 한 항목이 한 문단이라 키
  하나다. 문장 단위로 자르지 말 것 — 언어마다 문장을 나누는 지점이 달라서 조각을 주면 번역이
  안 된다. **`index.html`의 그 문단들은 비어 있다**: 마흔 몇 개를 마크업과 카탈로그에 두 번
  적으면 어긋날 자리가 마흔 몇 개고, 마크업 쪽은 어차피 화면에 안 나간다. 정확히는 56개 중
  **43개가 비어 있고, 짧은 제목·꼬리표 13개만 영어 그대로 남아 있다**(`guide.title`,
  `guide.q1.tag` 등). 낱말 몇 개짜리라 어긋날 여지가 없고, `FALLBACK`이 `en`이라 카탈로그가
  안 붙는 최악의 경우에도 읽히는 값이 된다. **비어 있지 않은 것을 보고 빠뜨린 줄로 읽지 말 것.**
