...
- **조합키는 macOS에서 Cmd다. 그것을 아는 곳이 `renderer/keys.ts` 하나뿐이어야 한다.**
  ...
  **사용자가 읽는 낱말은 카탈로그가 아니라 보간이 정한다.** `{{mod}}`를 쓰고, 값은 렌더러와
  ...
  나간다.
  **`{{mods}}`는 둘을 나란히 적는다**(`Ctrl / Cmd`, 고정 순서). 쓰는 곳은 가이드의
  단축키 목록 **하나뿐이고 그게 규칙이다** — 다른 문장은 "지금 이걸 누르세요"라서 없는 키가
  잡음이지만, 목록은 다른 기계 것을 대신 확인하려고 읽는 일이 많다. 순서를 실행 중인
  플랫폼에 맞춰 돌리면 그 두 읽기가 서로 어긋난다. 값은 `renderer/keys.ts`의 `BOTH_ACCELS`이고
  메인은 같은 문자열을 **손으로 한 번 더 적는다** — `keys.ts`가 `navigator`를 읽어서
  import할 수 없다.
