...
- **착수할 때 먼저 확인할 것 셋** (여기 숫자로 적지 않는 이유는 플랜 정책이 바뀌기 때문이다):
  이번 달 남은 횟수, `eas build --local`이 그 횟수를 쓰는지, EAS Update(OTA)가 별도 쿼터인지.
  **셋 다 확인하기 전에는 빌드 계획을 세우지 말 것** — 로컬 빌드와 OTA가 쿼터 밖이라면
  전략이 통째로 달라진다.
