...
| Ctrl / Cmd                        | Ctrl / Cmd                                      | 카탈로그에는 낱말을 적지 않고 `{{mod}}`를 쓴다. macOS는 `Cmd`, 그 밖은 `Ctrl`                                                                       |
...
| 빠른 입력                         | Quick capture                                   | 위젯이 여는 전용 화면. 거기서 적은 것은 다 꺼내기로 들어간다                                                                                        |
| 직접 적기                         | Type a task                                     | 위젯의 두 번째 문. `말로 적기`(`speech.start`)와 짝이고, 그래서 위젯은 그 키를 다시 쓰지 새로 적지 않는다                                           |
